...
class MemorySectionView(HardwareSectionView):
    ...
    def _render_hero_card(self, data: Dict[str, Any]) -> None:
        ...
        copy_btn.set_tooltip_text(_("Copy memory info"))
        ...
        if data.get("capacity"):
            array_items.append((_("Capacity"), data.get("capacity")))
        if data.get("modules_count"):
            array_items.append((_("Modules"), data.get("modules_count")))
        if data.get("slots"):
            array_items.append((_("Slots"), data.get("slots")))
        if data.get("max_module_size"):
            note = f" ({data.get('note')})" if data.get("note") else ""
            array_items.append((
                _("Max Module Size"),
                f"{data.get('max_module_size')}{note}",
            ))
        if data.get("ec"):
            array_items.append((_("ECC"), data.get("ec")))
    ...
    def _create_module_card(self, module: Dict[str, Any]) -> Gtk.Box:
        ...
        # Info rows
        info_items = [
            (_("Slot"), module.get("slot", "")),
            (_("Speed"), speed_display),
            (_("Volts"), module.get("volts", "")),
            (_("Manufacturer"), module.get("manufacturer", "")),
            (_("Part Number"), module.get("part_no", "")),
            (_("Serial"), module.get("serial", "")),
        ]
    ...
    def _render_swap_section(self) -> None:
        ...
        # Section title
        title = self.create_section_title(_("Swap / ZRAM"))
        self.append(title)
    ...
    def _render_swap_kernel_settings(self, swap_kernel: Dict[str, Any]) -> None:
        ...
        settings_header = Gtk.Label(label=_("Kernel Settings"))
        settings_header.add_css_class("device-title")
        settings_header.set_halign(Gtk.Align.START)
        settings_card.append(settings_header)
        
        settings_items = [
            (_("Swappiness"), swap_kernel.get("swappiness", "")),
            (_("Cache Pressure"), swap_kernel.get("cache_pressure", "")),
            (_("ZSwap"), swap_kernel.get("zswap", "")),
            (_("Compressor"), swap_kernel.get("compressor", "")),
        ]
